<template>
  <t-form ref="form" :class="['item-container', `register-${type}`]" :data="formData" :rules="FORM_RULES" label-width="0"
    @submit="onSubmit">
    ...
    <t-form-item class="check-container" name="checked">
      <t-checkbox v-model="formData.checked">我已阅读并同意 </t-checkbox> <span>Nodeserver服务协议</span> 和
      <span>隐私声明</span>
    </t-form-item>
...
const onSubmit = async (ctx: SubmitContext) => {
  if (ctx.validateResult === true) {
    if (!formData.value.checked) {
      MessagePlugin.error('请同意Nodeserver服务协议和隐私声明');
      return;
    }

    try {
      const account = formData.value.email as string; // 类型断言为string
      const password = formData.value.password as string;
      const phone = formData.value.password as string ||'null';
      const data = await Register(phone,account, password)
      if (data.token) {
